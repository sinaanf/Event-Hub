...
export default function Speakers() {
  ...
  async function fetchSpeakerDetail(id: string) {
    ...
    if (!res.ok) {
      toast({ title: "Could not load speaker details", variant: "destructive" });
      return;
    }
    const data = await res.json();
  ...
  const availableSessions = sessions.filter((s) => !assignedSessionIds.has(s.id) && s.event_id === selectedEventId);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[#F8F7F4]">
      ...
      <div className="px-8 pt-8 pb-5 border-b border-gray-100 bg-white shrink-0 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground" style={{ fontFamily: "Plus Jakarta Sans, sans-serif" }}>Speakers</h1>
                ...
                >
                  ...
                  {(speaker.email || speaker.phone) && (
                    <div className="mt-3 pt-3 border-t border-gray-50 flex gap-3">
                      {speaker.email && (
                        <a href={`mailto:${speaker.email}`} onClick={(e) => e.stopPropagation()} className="text-xs text-blue-600 hover:underline truncate">{speaker.email}</a>
                      )}
                      {speaker.phone && (
                        <a href={`tel:${speaker.phone}`} onClick={(e) => e.stopPropagation()} className="text-xs text-muted-foreground">{speaker.phone}</a>
                      )}
                    </div>
                  )}
                </div>
