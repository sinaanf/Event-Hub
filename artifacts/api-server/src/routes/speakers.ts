...
router.get("/:id", async (req: AuthRequest, res) => {
  ...
  if (error) return res.status(404).json({ error: "Speaker not found" });
  return res.json(data);
});
...
router.post("/", requireOrganiser(async (req, res) => {
  if (!supabase) return res.status(503).json({ error: "Supabase not configured" });
  const { name, job_title, company, headshot_url, email, phone, event_id } = req.body;
  if (!name) return res.status(400).json({ error: "name is required" });

  const { data, error } = await supabase
    .from("speakers")
    .insert([{ name, job_title, company, headshot_url, email, phone, event_id }])
    .select()
    .single();
  if (error) return res.status(500).json({ error: error.message });
...
router.patch("/:id", requireOrganiser(async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("speakers").update(payload).eq("id", req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
}));
...
router.post("/:id/sessions/:sessionId", requireOrganiser(async (req, res) => {
  if (!supabase) return res.status(503).json({ error: "Supabase not configured" });
  ...
  const { data: speaker, error: speakerErr } = await supabase
    .from("speakers").select("*").eq("id", speakerId).single();
  if (speakerErr || !speaker) return res.status(404).json({ error: "Speaker not found" });
...
router.delete("/:id/sessions/:sessionId", requireOrganiser(async (req, res) => {
  if (!supabase) return res.status(503).json({ error: "Supabase not configured" });
